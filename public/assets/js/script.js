...
$(document).on('change', '#country', function(e){
    var current_url = $('meta[name="base-url"]').attr('content');    
    var countryId = $(this).val();
    if (countryId != '') {
        $("#state").val('').trigger('change').prop('disabled', true);
        $.ajax({
            url:current_url +"/getState",
            type:'POST',
            headers: {
        ...
        $('#state').val('').trigger('change').html('<option value=""></option>');
        $("#state").prop('disabled', false);
    }
});
    // Get City List                   